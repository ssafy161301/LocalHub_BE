...
더미 게시글은 프론트엔드 작성 폼과 동일한 관광지, 문화시설, 축제, 여행코스, 레포츠, 숙박, 쇼핑, 맛집, 여행후기, 정보공유, 질문 카테고리를 순환하며 제목에 `[더미]`가 붙습니다. 공통 비밀번호는 `DUMMY_POST_PASSWORD` 환경변수로 지정하고, 값이 없으면 `dummy1234`를 사용합니다. 게시글이 한 건이라도 존재하면 자동 더미 게시글 적재를 건너뜁니다.
...
- 제목은 `[더미]`로 시작하며 프론트엔드 작성 폼과 동일한 11개 카테고리를 순환합니다.
