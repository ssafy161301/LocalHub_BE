...
지원 카테고리는 아래 7개이며, `count`는 현재 DB 건수다.
...
```json
{
  ...
  "data": [
    ...
    { "contentTypeId": "38", "name": "쇼핑", "count": 4368 }
  ],
  "message": null
}
```

응답 배열에는 항상 7개 카테고리가 모두 포함된다. DB에 데이터가 없는 카테고리의 `count`는 `0`이다.
...
| `category` | string | X | 없음 | 위 7개 카테고리명 중 하나 |
...
1. 현재 `message`에서 한글·영문·숫자 단어를 추출한다.
2. 일부 조사와 요청 표현을 제거하고 중복을 제거한 뒤 최대 10개 핵심어를 만든다.
3. 핵심어 중 하나라도 부분 일치하는 장소와 게시글 후보를 각각 최대 100개 조회한다.
4. 정확 검색 결과가 없는 자료 유형은 한글 자모 유사도 기반 퍼지 검색으로 오타 후보를 찾는다.
5. 장소는 제목·카테고리·주소, 게시글은 제목·카테고리·본문 일치에 가중치를 주어 정렬한다.
6. 상위 장소 10개와 게시글 10개를 OpenAI 참고자료로 전달한다.
7. `history`와 현재 메시지를 OpenAI Responses API에 전달한다.
8. 모델 답변과 검색된 참고자료를 함께 반환한다.
...
`history`는 모델의 대화 문맥으로만 사용하며 DB 검색어에는 포함하지 않는다. 서버에도 저장하지 않으므로 Vue에서 관리해 매 요청에 전달해야 한다. 기본 모델은 `gpt-5-mini`이며 `OPENAI_MODEL` 환경변수로 변경할 수 있다.
...
챗봇의 장소 reference에는 목록 응답과 달리 `contentTypeId`, `longitude`, `latitude`가 포함되지 않는다. 검색 결과가 없더라도 OpenAI 호출은 수행하며 성공하면 빈 reference 배열과 모델 답변을 `200`으로 반환한다.
...
```json
{
  ...
  "data": {
    ...
    "labels": ["관광지", "문화시설", "축제공연행사", "여행코스", "레포츠", "숙박", "쇼핑"],
    "datasets": [
      { "label": "데이터 수", "data": [783, 566, 201, 51, 126, 423, 4368] }
    ],
    "total": 6518
  },
  "message": null
